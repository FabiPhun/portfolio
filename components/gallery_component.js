...
function initLayout() {
  ...
  galleryLists.forEach(galleryList => {
    // Re-query images at run time instead of capturing a static NodeList in the
    // closure - the old version's `galleryImages` was fixed at initLayout()-time,
    // which is fine here but was easy to get stale once this fn is called more
    // than once against the same gallery element.
    const runLayout = () => {
    ...
    // ResizeObserver fires whenever the element's box actually changes size -
    // including the moment it goes from `display:none` (width 0, during the SPA
    // router's fade-in prep) to its real rendered width. A plain `window.resize`
    // listener never fires for that transition, which is why the layout only
    // ever looked right after the user manually resized the window.
    const ro = new ResizeObserver(() => runLayout());
    ro.observe(galleryList);
...
window.reinitGalleries = initGalleries;

window.removeGalleries = function removeGalleries() {
  _teardownObservers();
  const lightbox = document.querySelector(GALLERY_SETTINGS.lightboxSelector);
  if (lightbox) {
    lightbox.style.display = 'none';
